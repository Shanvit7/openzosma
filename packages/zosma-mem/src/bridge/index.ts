...
export interface MemoryBridge {
   ...
   */
  loadContext: (userMessage: string) => Promise<{ context: string; injectedIds: string[] }>
...
export const createMemoryBridge = (config: BridgeConfig): MemoryBridge => {
  ...
  const loadContext = async (userMessage: string): Promise<{ context: string; injectedIds: string[] }> => {
    const results = await engine.retrieve({ taskDescription: userMessage }, topK)

    if (results.length === 0) return { context: "", injectedIds: [] }
    ...
    return { context: formatContext(memories), injectedIds }
  }
  ...
  const recordUsage = async (
    entityId: string,
    signal: "used" | "ignored" | "influenced_decision",
  ): Promise<void> => {
    if (signal === "used") {
